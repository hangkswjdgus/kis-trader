...
QUOTE_EXCHANGE_BY_SYMBOL = {"TQQQ": "NAS", "SOXL": "AMS"}
ORDER_EXCHANGE_BY_QUOTE = {"NAS": "NASD", "NYS": "NYSE", "AMS": "AMEX"}

def quote_exchange_candidates(symbol: str, exchange: str) -> list[str]:
    symbol = symbol.upper()
    preferred = QUOTE_EXCHANGE_BY_SYMBOL.get(symbol, "NAS")
    given = (exchange or "AUTO").upper()
    base = [] if given in ("AUTO", "") else [given]
    for ex in [preferred, "NAS", "AMS", "NYS"]:
        if ex not in base:
            base.append(ex)
    return base

def to_order_exchange(quote_exchange: str) -> str:
    return ORDER_EXCHANGE_BY_QUOTE.get((quote_exchange or "").upper(), quote_exchange)
...
async def overseas_price(symbol: str, exchange: str) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        ...
        for ex in quote_exchange_candidates(symbol, exchange):
            params = {"AUTH": "", "EXCD": ex, "SYMB": symbol}
            ...
            if r.status_code >= 400:
                tried.append({"exchange": ex, "status": r.status_code, "text": r.text[:500]})
                continue
            ...
            try:
                ...
                if price > 0:
                    return {
                        "symbol": symbol,
                        "exchange": ex,
                        "order_exchange": to_order_exchange(ex),
                        "price": price,
                        "raw": out,
                        "fetched_at": now_iso(),
                    }
                ...
                pass
            tried.append({"exchange": ex, "response": data})

    raise HTTPException(status_code=502, detail=f"현재가 조회 실패: {symbol} 거래소 후보 {tried}")
...
async def place_overseas_order(side: Literal["buy", "sell"], symbol: str, exchange: str, qty: float, price: float, market_order: bool, fractional_order: bool = True) -> dict[str, Any]:
    ...
    # KIS 해외주식 실전 주문 TR ID. J... 계열은 EGW00356 타겟 서버 오류가 날 수 있음.
    tr_id = "TTTT1002U" if side == "buy" else "TTTT1006U"

    # KIS 미국주식 일반 주문은 시장가 코드가 제한적이라 화면의 시장가 주문은 보호지정가로 처리
    # 매수: 현재가보다 2% 높게, 매도: 현재가보다 2% 낮게 지정가 주문
    ord_dvsn = "00"
    if market_order:
        effective_price = price * (1.02 if side == "buy" else 0.98)
    else:
        effective_price = price
    order_price = f"{effective_price:.2f}"
    body = {
        "CANO": ACCOUNT_NO,
...
async def evaluate_strategy(trigger: str = "manual") -> dict[str, Any]:
    ...
    if st.position_qty > 0 and st.avg_price > 0:
        ...
        if pnl_pct >= cfg.target_profit_pct:
            result = await place_overseas_order("sell", cfg.symbol, quote.get("order_exchange", cfg.exchange), st.position_qty, current, cfg.market_order, cfg.fractional_order)
    ...
    result = await place_overseas_order("buy", cfg.symbol, quote.get("order_exchange", cfg.exchange), qty, current, cfg.market_order, cfg.fractional_order)
