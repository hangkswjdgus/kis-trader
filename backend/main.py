...
async def api_evaluate():
    ...
    if not cfg.enabled:
        add_log(st, "info", "자동매매 OFF: 평가만 수행", {"price": current})
        save_state(st)
    ...
    if st.position_qty > 0 and st.avg_price > 0:
        ...
        if pnl_pct >= cfg.target_profit_pct:
            ...
            st.last_buy_date = None
            st.cycle_no += 1
            add_log(st, "success", f"목표수익 도달: {sold_qty}주 전량 매도 주문", {"pnl_pct": pnl_pct, "target_price": target_price, "order": result})
            save_state(st)
    ...
    add_log(st, "success", f"정액 매수 주문: {cfg.symbol} {qty}주", {"price": current, "order": result})
    save_state(st)
